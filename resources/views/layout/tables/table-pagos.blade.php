            ...
            <td class="py-3">
              <div class="flex gap-4 items-center justify-center">
                ...
                @can('manage-resource', [$resource, 'delete'])
                  <button data-id="{{$fila[0]}}" class="delete-button text-gray-600 text-theme-sm dark:text-gray-400">
                    Eliminar
                  </button>
                @endcan

                @can('manage-resource', [$resource, 'view_details'])
                  <a href="{{ route('pago_detalles', [$fila[0]]) }}"
                    class="button text-gray-600 text-theme-sm dark:text-gray-400 inline-flex items-center gap-1">
                    Ver detalles
                    @if(isset($fila[8]) && $fila[8])
                      <span class="relative group">
                        <!-- Ícono SVG de triángulo amarillo con borde y símbolo de exclamación -->
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="inline-block">
                          <!-- Triángulo con relleno amarillo transparente y borde amarillo oscuro -->
                          <path d="M12 2L2 20H22L12 2Z" 
                                fill="rgba(234, 179, 8, 0.2)" 
                                stroke="#ca8a04" 
                                stroke-width="1.5" 
                                stroke-linejoin="round"/>
                          <!-- Símbolo de exclamación: negro en modo claro, blanco en modo oscuro -->
                        ...
                        <!-- Tooltip con fondo amarillo muy transparente y borde amarillo oscuro -->
                        <span class="absolute bottom-full right-0 mb-2 px-3 py-2 bg-yellow-300/30 backdrop-blur-sm text-gray-900 dark:text-gray-100 text-xs rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 shadow-lg w-48 border-2 border-yellow-600 dark:border-yellow-500">
                          <span class="block font-semibold">Detalle de pago rechazado.</span>
                          <span class="block">Haz clic para más información</span>
                          <span class="absolute top-full right-4 -mt-1 border-4 border-transparent border-t-yellow-600 dark:border-t-yellow-500"></span>
                        </span>
                      </span>
                    @endif
                  </a>
                ...
                @can('manage-resource', [$resource, 'edit'])
                  @if(isset($fila[6]) && $fila[6] == 1 && $fila[4] < $fila[7])
                    <a href="{{ route('pago_completar', [$fila[0]]) }}"
                      class="button text-gray-600 text-theme-sm dark:text-gray-400">
                      Completar Pago
                    </a>
                  @endif
                @endcan
